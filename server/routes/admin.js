...
const parent = taskClient.queuePath('zephyrd', 'us-central1', 'zephyr-queue-new');
console.log(parent);
const taskrequest = {
  name: parent,
...
router.get('/event', async (req, res) => {
  try {
    let vmData = [];
    let assemData = [];
    const tasks = await taskqueue.projects.locations.queues.tasks.list({parent});
    ...
    const runningVMs = newVMs.data.items.filter((vm) => vm.status != 'TERMINATED');
    ...
    if (status) {
      const inProcess = status.items.filter(assem => assem.ok === "ASSEMBLY_EXECUTING" )
    ...
    let eventInterval = setInterval(async () => {
      if (data.vms.length > 0 || data.assem.length >0 || Object.keys(data.tasks).length > 0 ){
...
router.get('/tasks', async (req, res) => {
  try {
    ...
    const tasks = await taskqueue.projects.locations.queues.tasks.list({parent}).data
    console.log(tasks);
