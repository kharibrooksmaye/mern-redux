...
const copyFile = async (userid, recordid) => {
	await zephyr
		...
		.copy(zephyr.bucket("zephyrinput")
...
const getFiles = async (userid, recordid) => {
  try {
    const results = await zephyr.bucket('zephyroutput').getFiles({ prefix: `${userid}/${recordid}` });
...
const publishMessage = async (data) => {
  ...
  try {
    const messageId = await pubSubClient.topic('zephyr-topic').publish(dataBuffer);
...
router.delete('/user/:id', async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({ count: result.deletedCount, message: `record and docs in ${req.params.id} deleted` });
  } catch (error) {
...
router.post('/:id/refresh', (req, res) => {
  ...
  transloadit.createAssembly(opts, (err) => {
    if (err) {
      console.log(err);
      throw err;
      ...
      res.status(200).json({ type: 'success', message: 'Analysis processing, please wait' });
    }
  });
});
...
router.put('/:id/upload', (req, res) => {
  ...
  const {
    user_id, encoded, thumb, info, recordId,
  } = req.body;
