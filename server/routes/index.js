...
router.get('/verify', (req, res) => {
  const authContact = req.query.authMethod === 'sms' ? `+1${req.query.authContact}` : req.query.authContact;
  const config = {
...
router.post('/checking', (req, res) => {
  ...
  const authContact = req.body.authMethod === 'sms' ? `+1${req.body.authContact}` : req.body.authContact;
  const { code } = req.body;
  ...
  bcrypt.hash(password, hashRounds).then((hashedPassword) => {
    ...
    User.findOne({
      ...
      .then(async (user) => {
        if (user) {
          res.send({
            error: 'A user already exists with these credentials. Please login or choose a different email address or username',
          });
        } else {
          try {
            const registeredUser = await newUser.save();
            const tokenUser = createTokenFromUser(registeredUser);
            const token = jwt.sign({ tokenUser, authorized: true }, 'testing out a secret', {
              expiresIn: 129600,
            });
            res.send({
              err: null,
      ...
      .catch(() => res.status(res.statusCode).send({ error: 'Error, user already exists' }));
  });
});
...
router.post('/create', async (req, res) => {
  ...
  if (existing) {
    res.send({ type: 'error', message: `A user with the email address ${email} already exists` });
  } else {
    try {
      await newUser.save();
...
router.get('/download', (req, res) => {
  const url = 'https://storage.cloud.google.com/zephyr-specimenuploads/Zephyr%20Questionnaire.pdf';
...
router.get('/activate', async (req, res) => {
  try {
    ...
    if (!user) res.status(401).json({ message: `The email address ${email} is not associated with any account. Contact Zephyr Analytics for support.` });
    if (!user.isActivated) res.status(200).json({ type: 'needs-activation', message: 'Your account has not been activated.' });
  } catch (error) {
    console.log(error);
  }
});
        ...
        bcrypt.compare(password, user.password, (err, match) => {
          if (!match) {
            ...
            const token = jwt.sign(
              {
                user: user._id,
                authContact: user.authMethod === 'sms' ? user.phoneNumber : user.email,
